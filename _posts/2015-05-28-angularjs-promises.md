...
Once again, that won't work. Now `getPoneys()` doesn't return anything anymore. The `return data` statement is not returning from `getPoneys()`. It's returning from the anonymous callback function
passed to `success()`. 
