...
you'll need to enable `propsDestructureTransform` in the compiler options to use this feature.
...
The ref sugar experiment (take 2) introduces new compiler macros (so you don't have to import them):
...
most commonly used APIs now have a `$` version that does the same thing, like `$ref()`, `$computed()` and `$shallowRef()`.
...
If you want to use it, you'll need to enable `refTransform` in the compiler options.
